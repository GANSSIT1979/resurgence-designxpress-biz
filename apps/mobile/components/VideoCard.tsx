...
export default function VideoCard({ item, isActive }: { item: any; isActive: boolean }) {
  ...
  useEffect(() => {
    if (isActive) {
      ...
      api.trackView?.(item.id).catch(() => undefined)
    } else {
  ...
  async function handleLike() {
    ...
    api.likePost?.(item.id).catch(() => undefined)
  }
  ...
  async function handleShop() {
    const url = product?.url || product?.checkoutUrl || `https://www.resurgence-dx.biz/shop/product/${product?.slug || ''}`
  ...
  return (
    <View style={{ height, backgroundColor: '#000' }}>
      <Video
        ref={ref}
        source={{ uri: item.videoUrl || item.mediaUrl || item.playbackUrl }}
        style={{ flex: 1 }}
      ...
      <View style={{ position: 'absolute', right: 16, bottom: 150, gap: 18 }}>
        <Pressable onPress={handleLike}>
          <Text style={{ color: liked ? '#ff3b5c' : '#fff', fontSize: 30 }}>♥</Text>
        </Pressable>
      ...
      <View style={{ position: 'absolute', bottom: 72, left: 16, right: 72 }}>
        <Text style={{ color: '#fff', fontSize: 16, fontWeight: '700' }}>
          @{item.creator?.slug || item.creatorSlug || 'creator'}
        </Text>
        <Text style={{ color: '#fff', fontSize: 18, marginTop: 6 }} numberOfLines={2}>
          {item.title || item.caption || 'Featured drop'}
        </Text>
        {product && (
          <Pressable onPress={handleShop} style={{ marginTop: 12, backgroundColor: '#fff', padding: 10, borderRadius: 999 }}>
            <Text style={{ color: '#000', fontWeight: '700' }}>
              Shop {product.name || product.title || 'this look'}
            </Text>
          </Pressable>
        )}
      </View>
