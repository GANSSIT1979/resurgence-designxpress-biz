...
export async function POST(
  ...
  try {
    ...
    const result = await unpublishCreatorPost(postId);
    if ('error' in result) return fail(result.error, 400);
    return ok({ action: 'unpublish', item: result.item });
  } catch (routeError) {
    const message = routeError instanceof Error ? routeError.message : 'Unable to move the creator post back to draft.';
