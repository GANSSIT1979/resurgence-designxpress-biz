...
export async function POST(
  ...
  try {
    ...
    const result = await archiveCreatorPost(postId);
    if ('error' in result) return fail(result.error, 400);
    return ok({ action: 'archive', item: result.item });
  } catch (routeError) {
    const message = routeError instanceof Error ? routeError.message : 'Unable to archive the creator post.';
