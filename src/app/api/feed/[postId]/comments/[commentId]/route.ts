...
import {
  deleteContentPostComment,
  updateContentPostComment,
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ postId: string; commentId: string }> }) {
  ...
  try {
    ...
    const result = await updateContentPostComment(actor, postId, commentId, payload);
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ postId: string; commentId: string }> }) {
  ...
  try {
    const result = await deleteContentPostComment(actor, postId, commentId);
    if ('error' in result && result.error) return fail(result.error, 400);
