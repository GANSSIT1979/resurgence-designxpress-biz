...
async function requireCashierFinance(request: NextRequest) {
  return requireApiPermission(request, 'cashier.finance.manage');
}

export async function GET(request: NextRequest, { params }: Params) {
  const auth = await requireCashierFinance(request);
